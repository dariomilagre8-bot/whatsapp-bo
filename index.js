...
// Pega na lista de supervisores e limpa
...
async function sendWhatsAppMessage(number, text) {
  try {
    // Tenta detetar se é um ID longo (LID) ou número normal
    let formattedNumber = cleanNumber(number);
    
    // Se for número normal de Angola (9 dígitos) ou internacional, adiciona sufixo
    // Se for LID gigante (como o 2513...), temos de ter cuidado, mas por norma adicionamos @s.whatsapp.net
...
app.post('/', async (req, res) => {
  try {
    ...
    if (SUPERVISOR_NUMBERS.includes(senderNum)) {
      console.log('👑 Supervisor reconhecido.');
      ...
      if (action) {
        ...
        if (!targetClientNum) {
            ...
            if (pendingList.length === 1) targetClientNum = pendingList[0];
            else if (pendingList.length > 1) {
                await sendWhatsAppMessage(remoteJid, `⚠️ Tenho ${pendingList.length} pedidos. Digite "sim 9xxxx".`);
        ...
        if (!pedido) {
            await sendWhatsAppMessage(remoteJid, "⚠️ Cliente não encontrado.");
            return res.status(200).send('OK');
        }

        if (action === 'approve') {
            await sendWhatsAppMessage(remoteJid, "🔄 A processar...");
            ...
            if (profile) {
                ...
                await sendWhatsAppMessage(remoteJid, `✅ Conta enviada!`);
            } else {
                await sendWhatsAppMessage(targetClientNum, "✅ Recebido! Supervisor enviará a conta manualmente.");
                await sendWhatsAppMessage(remoteJid, `⚠️ *SEM STOCK AUTOMÁTICO* para ${targetClientNum}. Envie manualmente!`);
                delete pendingVerifications[targetClientNum];
                delete clientStates[targetClientNum]; 
            }
        } else {
            await sendWhatsAppMessage(targetClientNum, "❌ Comprovativo inválido.");
            delete pendingVerifications[targetClientNum];
            delete clientStates[targetClientNum];
    ...
    // --- CLIENTE ---
    // (Se o número for estranho e longo, provavelmente é o supervisor noutro dispositivo. Ignora para não dar erro)
    if (senderNum.length > 15) {
        console.log(`⚠️ Ignorando ID longo desconhecido: ${senderNum}`);
        return res.status(200).send('OK');
    }
    ...
    if (clientStates[senderNum].step === 'aguardando_comprovativo') {
        if (isDoc && messageData.message.documentMessage.mimetype === 'application/pdf') {
            ...
            for (const sNum of SUPERVISOR_NUMBERS) await sendWhatsAppMessage(sNum, msgSuper);
  ...
  } catch (error) { console.error('ERRO GERAL:', error); res.status(200).send('Erro'); }
});

app.listen(port, '0.0.0.0', () => console.log(`Bot v4.0 (Logs Detalhados) rodando na porta ${port}`));