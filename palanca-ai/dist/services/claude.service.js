...
function buildSystemPrompt(botType) {
...
function conversationToMessages(log) {
    return log.map((t) => ({
        role: t.role,
        content: t.content,
    }));
...
function parseFinalResponse(text) {
    ...
    try {
        ...
        if (parsed.status !== 'APROVADO' && parsed.status !== 'REPROVADO')
            return null;
        return {
            type: 'final',
...
export class ClaudeService {
    ...
    async generateNextInteraction(history, botType, options) {
        ...
        if (forceFinal) {
            ...
            const extra = lastUser
                ...
                : [
                    {
                        role: 'user',
                        content: 'Avalia a conversa e responde APENAS com o objeto JSON final (status, duration_turns, summary, failures, relatorio_markdown).',
                    },
                ];
            messages.push(...extra);
        }
    ...
    async getNextMessage(context, conversationLog) {
        const ctx = context;
    ...
    async evaluateSession(context, conversationLog) {
        const ctx = context;
        const log = conversationLog;
        const botType = ctx?.botContext?.tipoBot ?? 'generic';
        const history = conversationToMessages(log);
