...
telegram.onCommand('testar_bot', async (msg, args) => {
    ...
    try {
        ...
        await telegram.sendAdminAlert(`Teste iniciado: ${session.id}\nAlvo: ${session.targetWhatsApp}\nBot: ${session.botContext.tipoBot}`);
    }
    catch (err) {
        await telegram.sendAdminAlert(`Erro ao iniciar teste: ${err instanceof Error ? err.message : String(err)}`);
    }
});
